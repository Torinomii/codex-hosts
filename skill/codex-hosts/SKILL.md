...
Install the complete Skill as one directory symbolic link to the source Skill directory; bin\codex-hosts.exe resolves to the release build. From a built source checkout, run `pwsh -NoProfile -File .\scripts\install-local-skill.ps1`; from a downloaded or locally prepared distribution ZIP, run `pwsh -NoProfile -File .\install-local-skill.ps1 -ReleasePackage` after extraction. Keep the source or extraction directory in place so the links remain valid. See the [project README](https://github.com/Torinomii/codex-hosts#installation) for the full setup.
