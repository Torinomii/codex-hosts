...
- CSV テンプレートから複数のホストをインポートし、パスワードと秘密鍵パスフレーズを Windows 資格情報マネージャーへ移せます。インポート後に機密情報を含む元ファイルを直ちに削除でき、表計算ソフトが追加した列は無視されます。
- 保存済みホストを一括テストし、ホストごとに 10 秒で打ち切り、成功と失敗を色で明確に表示します。
- 複数のホストを選択して一括削除または認証情報なしの CSV エクスポートができます。フォルダーへのエクスポートは既存ファイルを上書きせず、新しい連番ファイル名を使います。
...
パスワードや秘密鍵パスフレーズを、会話、コマンドライン引数、JSON、スクリプト、ログに含めないでください。唯一のファイル例外は、ユーザーが明示的に選択してインポートする CSV です。アプリはこれを機密ファイルとして扱い、資格情報を Windows 資格情報マネージャーへ移した直後に削除するか確認します。
